<script setup>
    ...
    const chatStatus = (logs,error = null) => {
        let status = 'sent';
        ...
        logs.forEach(log => {
            ...
            if (logStatus === 'failed') {
                status = 'failed';
                error_code = metadata.errors ? metadata.errors[0].code : '';
                error_message = metadata.errors ? metadata.errors[0].message : '';
                return;
    ...
    const productPrice = (products) => {
        var price = products.map(e => {return e.item_price * e.quantity;});
        var total = price.reduce((pervItem,item) => {
            return pervItem + item;
        })
        return products[0].currency + ' ' + total;
    }

    const emit = defineEmits(['shippingDetails']);

    const openModal = (data) => {
        emit('shippingDetails', {details:data});
    }
    ...
    const sym = (currency) => {
        return new Intl.NumberFormat(getLocale(), { style: 'currency', currency }).formatToParts(1).find(x => x.type === "currency").value;
    }
    ...
    const getOrderItemsName = (items) =>{
        var names = [];
        items.forEach((e)=>{ names.push(e.name) });
...
<template>
    <div 
        class="rounded-lg my-1 text-sm flex flex-col relative"
        :class="props.type === 'outbound' ? 'ml-auto rounded-tr-none bg-[#d8fad4] speech-bubble-right' : 'mr-auto rounded-tl-none bg-white speech-bubble-left', (props.type === 'outbound' && JSON.parse(content.metadata).type === 'order') || (JSON.parse(content.metadata).type === 'interactive' && JSON.parse(content.metadata).interactive.type  === 'order_details') ? '' : 'p-2'">
        <div>
            ...
            <div v-if="JSON.parse(content.metadata).type === 'interactive'" :class="[JSON.parse(content.metadata).interactive.type  !== 'order_details' ? 'max-w-[300px]' : '']">
                ...
                <div v-if="JSON.parse(content.metadata).interactive.type  === 'nfm_reply'" class="border-2 hover:bg-gray-200 cursor-pointer border-gray-300 flex items-center p-1 rounded" style="line-height: 30px;word-wrap: break-word;" @click="openModal(JSON.parse(content.metadata).interactive.nfm_reply.response_json)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-file-text"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
                    <p class="normal-case whitespace-pre-wrap ml-2 mr-1" style="line-height: 20px;">{{ 'Shipping Details Submitted' + " for #" + content.order_id }}</p>
                </div>
                <div v-if="JSON.parse(content.metadata).interactive.type  === 'order_details'" class="cursor-pointer" style="max-width: 350px;">
                    <div class="p-1" style="line-height: 30px;" v-if="JSON.parse(content.metadata).interactive?.header?.image?.link">
                        <img class="bg-white rounded-[0.5rem]" draggable="true" :src="JSON.parse(content.metadata).interactive.header.image.link" style="">
                    </div>
                    <div>
                        <div class="p-2" v-if="JSON.parse(content.metadata).interactive?.action?.parameters">
                            <div class="mt-2">
                                <div class="text-gray-500 uppercase" style="text-transform: uppercase;letter-spacing: 1.5px;font-weight: 500;line-height: 1.6;font-size: 0.625rem;">{{ 'Order #' +  JSON.parse(content.metadata).interactive.action.parameters.reference_id }}</div>
                            </div>
                            <div class="border-black border-dashed border-t mt-2 p-2"></div>
                            <div class="flex gap-2 items-center">
                                <div>
                                    <img class="bg-white rounded-[0.5rem]" :src="JSON.parse(content.metadata).interactive.header.image.link" alt="" width="48px" height="48px">
                                </div>
                                <div class="overflow-hidden">
                                    <span class="block overflow-ellipsis overflow-hidden relative whitespace-no-wrap whitespace-nowrap" style="max-width: 260px;">{{ getOrderItemsName(JSON.parse(content.metadata).interactive.action.parameters.order.items) }}</span>
                                    <p class="text-gray-500 " style="font-weight: normal; font-size: 12px;" v-if="JSON.parse(content.metadata).interactive.action.parameters.order.items.length == 1">{{ 'Quantity ' + getOrderQuantity(JSON.parse(content.metadata).interactive.action.parameters.order.items) }}</p>
                                    <p class="text-gray-500 " style="font-weight: normal; font-size: 12px;" v-else="JSON.parse(content.metadata).interactive.action.parameters.order.items.length > 1">{{ JSON.parse(content.metadata).interactive.action.parameters.order.items.length + ' items'}}</p>
                                </div>
                            </div>
                            <div class="border-black border-dashed border-b mb-2 p-2"></div>
                            <div class="mt-2 flex justify-between">
                                <span>Total</span>
                                <span>{{ sym(JSON.parse(content.metadata).interactive.action.parameters.currency) + ' ' + JSON.parse(content.metadata).interactive.action.parameters.total_amount.value / JSON.parse(content.metadata).interactive.action.parameters.total_amount.offset }}</span>
                            </div>
                        </div>
                    </div>
                    <div class="mt-2 px-2 flex justify-between" v-if="JSON.parse(content.metadata).interactive?.body">
                        <span class="">{{ JSON.parse(content.metadata).interactive.body.text }}</span>
                    </div>
                    <div class="mt-2 px-2 pb-2 flex justify-between" v-if="JSON.parse(content.metadata).interactive?.footer">
                        <span class="text-gray-500">{{ JSON.parse(content.metadata).interactive.footer.text }}</span>
                    </div>
                    <div class="flex items-center justify-center border-t border-gray-300" v-if="JSON.parse(props.content.metadata).interactive.action.name" style="line-height: 30px;">
                        <button class="opacity-50 capitalize" :disabled="true">{{ (JSON.parse(props.content.metadata).interactive.action.name).replaceAll('_', ' ') }}</button>
                    </div>
                </div>
            </div>
            ...
            <!--Orders formatting-->
            <div v-else-if="JSON.parse(content.metadata).type === 'order'" class="" >
                <div style="line-height: 30px;" v-if="props.type === 'inbound'">
                    <p class="normal-case whitespace-pre-wrap">{{ 'Order Placed' }}</p>
                    <div class="flex items-center gap-2 mx-4">
                        ...
                        <p class="normal-case whitespace-pre-wrap">{{ JSON.parse(content.metadata).order.product_items.length + ' items' + " #" + content.order_id }}</p>
                    </div>
                    <p class="normal-case whitespace-pre-wrap mx-4" v-if="JSON.parse(content.metadata).order.product_items.length">{{ productPrice(JSON.parse(content.metadata).order.product_items) + ' (estimated total)' }}</p>
                </div>
                <div style="line-height: 30px;max-width: 350px;" v-if="props.type === 'outbound'">
                    <div class="p-2" v-if="orgProductSettings" style="word-wrap: break-word;">
                        <p class="normal-case whitespace-pre-wrap">{{ orgProductSettings?.header }}</p>
                        <div>{{ orgProductSettings?.body }}</div>
                        <a rel="stylesheet" class="flex items-center space-x-1 underline" :href="'//' + orgProductSettings?.footer" target="_blank">{{ orgProductSettings?.footer }}</a>
                    </div>
                    <div class="flex items-center justify-center border-t border-gray-300"  v-if="orgProductSettings">
                        <button class="opacity-50" :disabled="true">{{ orgProductSettings?.button_text }}</button>
                    </div>
                    <div class="p-2" v-if="!orgProductSettings">{{ 'Flow Message Sent!' }}</div>
                </div>
            </div>

            <!--Timestamp-->
            <div v-if="props.type === 'outbound' && content.user" class="mt-2 mb--2" :class="(props.type === 'outbound' && JSON.parse(content.metadata).type === 'order') || (JSON.parse(content.metadata).type === 'interactive' && JSON.parse(content.metadata).interactive.type  === 'order_details') ? 'p-2' : ''">
                <span  class="text-gray-500 text-xs text-right leading-none">Sent By: <u>{{ content.user?.first_name + ' ' + content.user?.last_name }}</u></span>
            </div>
            <div class="flex items-center justify-between space-x-4" :class="props.type === 'outbound' && content.user ? '' : 'mt-2',(props.type === 'outbound' && JSON.parse(content.metadata).type === 'order') || (JSON.parse(content.metadata).type === 'interactive' && JSON.parse(content.metadata).interactive.type  === 'order_details') ? 'p-2' : ''">
                ...
                <span class="relative group cursor-pointer" :class="chatStatus(content.logs) === 'read' ? 'text-blue-500' : 'text-gray-500'">
                    <!-- Tooltip text -->
                    <div class="absolute capitalize hidden group-hover:block bg-white text-gray-600 text-xs rounded-sm py-1 px-2 bottom-full mb-1 whitespace-no-wrap right-0" style="width: max-content;">
                        {{ $t(chatStatus(content.logs,true)) }}
                    </div>
