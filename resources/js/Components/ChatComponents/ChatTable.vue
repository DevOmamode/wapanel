<script setup>
    ...
    const chatLink = (contact) => {
        return props.status != 'all' ? `/chats/${contact.uuid}?status=${props.status}&page=${props.rows.meta.current_page}` : `/chats/${contact.uuid}?page=${props.rows.meta.current_page}` ;
    } 
...
<template>
    ...
    <div class="flex-grow overflow-y-auto h-[65vh]" ref="scrollContainer">
        <Link :href="chatLink(contact)" class="block border-b group-hover:pr-0" :class="contact.unread_messages > 0 ? 'bg-green-50' : ''" v-for="(contact, index) in rows.data" :key="index">
            <div class="flex space-x-2 hover:bg-gray-50 cursor-pointer py-3 px-4">
                ...
                <div class="w-[85%]">
                    <div class="flex justify-between">
                        ...
                        <span class="self-center text-slate-500 text-xs">{{ contact.last_chat ? formatTime(contact.last_chat?.created_at) :'' }}</span>
                    </div>
                    <div v-if="contact.last_chat?.deleted_at === null" class="flex justify-between">
                        ...
                        <div v-if="contentType(contact.last_chat?.metadata) ==='interactive' && content(contact.last_chat?.metadata)?.interactive?.type !== 'nfm_reply' && content(contact.last_chat?.metadata)?.interactive?.type !== 'order_details'" class="text-slate-500 text-xs truncate self-end"> {{ content(contact.last_chat?.metadata).interactive?.button_reply?.title || content(contact.last_chat?.metadata).interactive?.list_reply?.title }}</div>
                        <div v-if="contentType(contact.last_chat?.metadata) ==='interactive' && content(contact.last_chat?.metadata)?.interactive?.type  === 'nfm_reply'" class="text-slate-500 text-xs truncate self-end"> {{ 'Shipping Details Submitted ' }}</div>
                        <div v-if="contentType(contact.last_chat?.metadata) ==='interactive' && content(contact.last_chat?.metadata)?.interactive?.type  === 'order_details'" class="text-slate-500 text-xs truncate self-end"> {{ content(contact.last_chat?.metadata).interactive.action.parameters.order.items[0].name + ' · ' + sym(content(contact.last_chat?.metadata).interactive.action.parameters.currency) + content(contact.last_chat?.metadata).interactive.action.parameters.total_amount.value / content(contact.last_chat?.metadata).interactive.action.parameters.total_amount.offset }}</div>
                        ...
                        <div v-if="contentType(contact.last_chat?.metadata) ==='order' && contact.last_chat?.type === 'inbound'" class="text-slate-500 text-xs truncate self-end"> {{ 'Order Placed' }}</div>
                        <div v-if="contentType(contact.last_chat?.metadata) ==='order' && contact.last_chat?.type === 'outbound'" class="text-slate-500 text-xs truncate self-end"> {{ orgProductSettings?.header }}</div>
