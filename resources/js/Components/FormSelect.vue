<script setup>
    ...
    const contactLength = computed(() => {
      return props.options.filter(option => {
        ...
        return props.modelValue === option.value;
      }).map(option => option.contactCount).join(', ');
    })
</script>
<template>
    <div :class="className">
        ...
        <div class="">
            <Listbox 
                ...
                <div class="relative">
                    ...
                    >
                    <span class="block truncate" v-if="label">{{ label }}
                      <span v-if="contactLength && contactCount == true">{{ '&nbsp;&nbsp;&nbsp;(' + contactLength +')' }}</span>
                    </span>
                    ...
                    >
                    <ListboxOptions
                        class="z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm" :class="[optionClassName, props.relativeFlag == true ? 'relative' : 'absolute']"
                    >
                        <ListboxOption
                        ...
                        <li :class="[active ? 'bg-amber-100 text-amber-900' : 'text-gray-900','relative cursor-default select-none py-2 pl-10 pr-4']">
                            <span :class="[selected ? 'font-medium' : 'font-normal', 'block truncate']">
                              {{ option.label }}<span v-if="contactCount == true">{{ (option.contactCount ? '&nbsp;&nbsp;&nbsp;(' + option.contactCount +')' : '&nbsp;&nbsp;&nbsp;(0)') }}</span>
                            </span>
