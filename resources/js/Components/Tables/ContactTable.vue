<script setup>
    ...
    import LabelStatusToggle from '../ContactComponents/LabelStatusToggle.vue';
    const { isOpenAlert, openAlert, confirmAlert } = useAlertModal();

    const props = defineProps(['rows', 'filters', 'type','allContactLabels','contactLabelIds','labelState','rowCount']);
    ...
    const label = trans('Assign labels');
    const isOpenFormModal = ref(false);
    const assignLabelForm = useForm({
    ...
    const runSearch = () => {
        ...
        router.visit(props.type === 'contact' ? '/contacts' : props.type === 'group' ? '/contact-groups' : '/contact-labels' , {
            method: 'get',
            data: filteredParams,
        })
    }
    ...
    function saveCheckedItems() {
        if(props.type === 'contact'){
            ...
            localStorage.setItem('checkedLabels', JSON.stringify(checkedGroups.value));
        }
    }
    ...
    function loadCheckedItems() {
        if(props.type === 'contact'){
            ...
            const savedItems = localStorage.getItem('checkedLabels');
            checkedLabels.value = savedItems ? JSON.parse(savedItems) : [];
        }
    }
    ...
    function updateCheckedItems(uuid, isChecked) {
        if(props.type === 'contact'){
            ...
            const index = checkedLabels.value.indexOf(uuid);

            if (isChecked && index === -1) {
                checkedLabels.value.push(uuid);
            } else if (!isChecked && index !== -1) {
                checkedLabels.value.splice(index, 1);
            }
        }
    ...
    function applyCheckedState() {
        props.rows.data.forEach(row => {
            row.isChecked = props.type === 'contact' ? checkedContacts.value.includes(row.uuid) : props.type === 'group' ? checkedGroups.value.includes(row.uuid) : checkedLabels.value.includes(row.uuid);
        });
        updateBulkCheckboxState();
        updateSelectedCount();
    }
    ...
    function updateSelectedCount() {
        selectedCount.value = props.type === 'contact' ? checkedContacts.value.length : props.type === 'group' ? checkedGroups.value.length : checkedLabels.value.length;
    }

    function deleteItems(value){
        const itemsToDelete = props.type === 'contact' ? checkedContacts.value : props.type === 'group' ? checkedGroups.value : checkedLabels.value;
        ...
        router.visit(props.type === 'contact' ? '/contacts' : props.type === 'group' ? '/contact-groups' : '/contact-labels', {
            ...
            preserveState: true,
            onSuccess: () => {
                localStorage.removeItem(props.type === 'contact' ? 'checkedContacts' : props.type === 'group' ? 'checkedGroups' : 'checkedLabels');
                if(props.type === 'contact'){
    ...
    const contactListDataStore = (array) => {
        assignLabelForm.selectedLabels = [];
        array.forEach(element => {
            assignLabelForm.selectedLabels.push(element.id);
        });
    }

    const submitForm = async () => {
        assignLabelForm.processing = true;
        if(!assignLabelForm.selectedLabels.length){
            assignLabelForm.errors.content = 'Label Field is required';
            assignLabelForm.processing = false;
        }
        assignLabelForm.selectedContacts = checkedContacts;
        if(!assignLabelForm.selectedContacts.length){
            assignLabelForm.errors.content = 'Contact Field is required';
            assignLabelForm.processing = false;
        }
        if(!assignLabelForm.selectedLabels.length || !assignLabelForm.selectedContacts.length) {
            return false;
        }else{
            assignLabelForm.errors.content = '';
        }
        assignLabelForm.post('/contact-labels/bulk_contact/store', {
            onFinish: () => {
                assignLabelForm.processing = false;
    ...
    const emitStatus = (type) => {    
        if(!params.value.status || params.value.status == 'all'){
            params.value.status = type;
        }else if(type == 'all'){
            params.value.status = type;
        }else{
            if(!params.value.status.split(',').filter(e => {return e == type}).length && type != 'all'){
                params.value.status += `,${type}`;
            }
        }
        runSearch();
...
<template>
    ...
    <div class="flex justify-between px-4 border-b">
        ...
        <div class="flex items-center">
            <div v-if="props.type === 'contact'">
                <LabelStatusToggle :rows="allContactLabels" :filters="filters" :status="labelState" :rowCount="rowCount" @status="emitStatus"></LabelStatusToggle>
            </div>
                ...
                <Dropdown :align="'right'" class="mt-2">
                    ...
                    <template #items>
                        <DropdownItemGroup>
                            ...
                            <DropdownItem as="button"><a :href="type === 'contact' ? '/contacts/export' : type === 'group' ? '/contact-groups/export' : '/contact-labels/export'" class="w-full h-full">{{ $t('Export to xlsx') }}</a></DropdownItem>
                            <DropdownItem v-if="type === 'contact' && allContactLabels.length" as="button" @click="isOpenFormModal = true;">{{ $t('Assign labels') }}</DropdownItem>
    ...
    <div class="h-[5vh]">
        <div class="flex justify-between text-sm border-b">
            <Link href="/contacts" class="pt-3 w-1/2 text-center pb-1 hover:bg-slate-50" :class="{ 'bg-gray-50 border-b-2 border-slate-700': $page.url.startsWith('/contacts') }">{{ $t('Contacts') }}</Link>
            ...
            <Link href="/contact-labels" class="pt-3 w-1/2 text-center pb-1 hover:bg-slate-50" :class="{ 'bg-gray-50 border-b-2 border-slate-700': $page.url.startsWith('/contact-labels') }">{{ $t('Labels') }}</Link>
        </div>
    </div>
    <div class="flex-grow overflow-y-auto h-[65vh]" ref="scrollContainer">
        ...
        <div v-else-if="type === 'group' || type === 'label'" @click="getRow(row.uuid)" class="flex space-x-2 hover:bg-gray-50 cursor-pointer px-4 py-3 border-b" :class="row.isChecked ? 'bg-gray-50' : ''" v-for="(row, key) in rows.data" :key="key">
            <div>
    ...
    <ContactImportModal :type="type" v-model:modelValue="isOpenModal" :allContactLabels="allContactLabels" :contactLabelIds="contactLabelIds"/>
    <Modal :label="label" :isOpen=isOpenFormModal>
        <div class="mt-5 grid grid-cols-1 gap-x-6 gap-y-4">
            <form @submit.prevent="submitForm()" class="">
                <MultiSelectComboBox :allData="props.allContactLabels" :name="$t('Label')" :type="'allContactLabels'" :selectedData="contactLabelIds" @contactListDataForward="contactListDataStore" :position="true"/>
                <div v-if="assignLabelForm.errors.content" class="form-error text-[#b91c1c] text-xs">{{ assignLabelForm.errors.content }}</div>
                <div class="mt-4 flex">
                    ...
                    <button :class="['inline-flex justify-center rounded-md border border-transparent bg-primary px-4 py-2 text-sm text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2', { 'opacity-50': assignLabelForm.processing }]" :disabled="assignLabelForm.processing">
                        <svg v-if="assignLabelForm.processing" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path fill="currentColor" d="M12 2A10 10 0 1 0 22 12A10 10 0 0 0 12 2Zm0 18a8 8 0 1 1 8-8A8 8 0 0 1 12 20Z" opacity=".5"/><path fill="currentColor" d="M20 12h2A10 10 0 0 0 12 2V4A8 8 0 0 1 20 12Z"><animateTransform attributeName="transform" dur="1s" from="0 12 12" repeatCount="indefinite" to="360 12 12" type="rotate"/></path></svg>
    ...
    <AlertModal 
        ...
        :label = "$t('Delete ' + (props.type === 'contact' ? 'Contacts' : (props.type === 'group' ? 'Groups' : 'Labels')))" 
        :description = "$t('Are you sure you want to delete the '+ (props.type === 'contact' ? ' contacts' : (props.type === 'group' ? 'groups' : 'labels'))+'? This action can not be undone')"
    />
</template>
