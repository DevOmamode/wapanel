<script setup>
    ...
    const props = defineProps(['contactGroups', 'contact', 'fields', 'locationSettings','allContactLabels','contactLabelIds']);
    ...
    const contactListDataStore = (array) => {
        form.contactLabelIds = [];
        array.forEach(element => {
            form.contactLabelIds.push(element.id);
        });
    }
    ...
    <div class="flex justify-center md:h-[90vh] md:overflow-y-scroll">
        <form @submit.prevent="submitForm()" class="w-[30em]">
            ...
            <div class="grid gap-x-6 gap-y-4 sm:grid-cols-6 pb-6 border-b">
                ...
                <MultiSelectComboBox :allData="props.allContactLabels" :name="$t('Label')" :type="'allContactLabels'" :selectedData="contactLabelIds" @contactListDataForward="contactListDataStore" :class="'sm:col-span-6'"/>
            </div>
