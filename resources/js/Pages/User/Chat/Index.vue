    ...
    <AppLayout v-slot:default="slotProps">
        <div class="md:flex md:flex-grow md:overflow-hidden">
            <div class="md:w-[30%] md:flex flex-col h-full bg-white border-r border-l" :class="contact ? 'hidden' : ''">
                <ChatTable :rows="rows" :filters="props.filters" :rowCount="props.rowCount" :ticketingIsEnabled="ticketingIsEnabled" :status="props?.status" :chatSortDirection="props.chat_sort_direction" :orgProductSettings="orgProductSettings"/>
            </div>
            <div class="min-w-0 bg-cover flex flex-col chat-bg"  :class="contact ? 'h-screen md:w-[70%]' : 'md:h-screen md:w-[70%]'">
                ...
                <div v-if="contact && !displayTemplate" class="flex-1 overflow-y-auto" ref="scrollContainer2">
                    <ChatThread v-if="!displayContactInfo && !loadingThread && !displayTemplate" :rows="chatThread" :orgProductSettings="orgProductSettings" />
                    <Contact v-if="displayContactInfo && !displayTemplate" class="bg-white h-full" :fields="props.fields" :contact="contact" :locationSettings="props.locationSettings" :contactLabels="contactLabels" />
                </div>
...
<script setup>
    ...
    const props = defineProps(['rows', 'rowCount', 'pusherSettings', 'organizationId', 'isChatLimitReached', 'toggleNavBar', 'state', 'demoNumber', 'settings', 'status', 'chatThread', 'addon', 'contact', 'ticket', 'chat_sort_direction', 'filters', 'templates', 'fields', 'locationSettings','orgProductSettings','contactLabels']);
