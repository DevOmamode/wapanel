<template>
    <AppLayout>
        <div class="bg-white md:bg-inherit md:flex md:flex-grow capitalize">
            <div class="md:w-[30%] md:flex flex-col h-full bg-white border-r border-l" :class="group ? 'hidden' : ''">
                ...
                <ContactTable :rows="props.rows" :filters="props.filters" :type="'group'" @callback="handleGroup" :allContactLabels="allContactLabels"/>
            </div>
    ...
    <ContactImportModal :type="'group'" v-model:modelValue="isOpenModal" :allContactLabels="allContactLabels"/>
</template>
<script setup>
    ...
    const props = defineProps({ rows: Object, filters: Object, rowCount: Number, group: Object, allContactLabels: Object });
