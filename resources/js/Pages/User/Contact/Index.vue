<template>
    <AppLayout>
        <div class="md:bg-inherit bg-white md:flex md:flex-grow capitalize">
            <div class="md:w-[30%] flex-col h-full bg-white border-r border-l md:flex" :class="$page.url === '/contacts/add' || contact ? 'hidden' : ''">
                ...
                <ContactTable :rows="props.rows" :filters="props.filters" :type="'contact'" @callback="handleContact" :allContactLabels="allContactLabels" :contactLabelIds="contactLabelIds" :labelState="labelState" :rowCount="rowCount"/>
            </div>
            <div class="md:w-[70%] bg-cover md:h-[100vh] md:overflow-y-hidden">
                <div v-if="contact">
                    <ContactInfo v-if="!editContact" class="pt-20" :contact="contact" :fields="props.fields" :locationSettings="locationSettings" :contactLabels="contactLabels"/>
                    <ContactForm v-else :contactGroups="props.contactGroups" :contact="props.contact" :fields="props.fields" :locationSettings="locationSettings" :allContactLabels="props.allContactLabels" :contactLabelIds="contactLabelIds"/>
                </div>
                <div v-else>
                    <div v-if="$page.url === '/contacts/add'">
                        <ContactForm :contactGroups="props.contactGroups" :contact="props.contact" :fields="props.fields" :locationSettings="locationSettings" :allContactLabels="props.allContactLabels" :contactLabelIds="contactLabelIds"/>
                    </div>
                    <div v-else>
    ...
    <ContactImportModal :type="'contact'" v-model:modelValue="isOpenModal" :allContactLabels="allContactLabels" :contactLabelIds="contactLabelIds"/>
</template>
<script setup>
    ...
    const props = defineProps({ rows: Object, filters: Object, rowCount: Number, contactGroups: Object, contact: Object, editContact: Boolean, fields: Object, locationSettings: String, flash: Object, allContactLabels: Object, contactLabelIds:Array, contactLabels:String,labelState:String});
