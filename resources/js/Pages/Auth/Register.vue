    ...
    <div class="flex min-h-screen align-center justify-center bg-gray-50">
        <div class="">
            <div class="w-[21em] md:w-[26.5em] bg-white shadow-md px-5 pt-10 pb-10 rounded-2xl mb-20 mt-20">
                ...
                <h1 class="text-2xl text-center text-gray-600">{{ $t('Create account') }}</h1>
                <form @submit.prevent="submitForm()" class="mt-5 pb-10">
                    <div class="mt-5 grid gap-x-6 gap-y-4 grid-cols-6">
                        <FormInput v-model="form.first_name" inputIcon='<i class="pi pi-user"></i>' :inputClass="`py-2.5 focus:border-green-200 focus:border-2 focus:ring-green-500 focus:ring-2`" :name="$t('First name')+':'" :error="form.errors.first_name" :type="'text'" :class="'col-span-6 md:col-span-3'"/>
                        <FormInput v-model="form.last_name" inputIcon='<i class="pi pi-user-plus"></i>' :inputClass="`py-2.5 focus:border-green-200 focus:border-2 focus:ring-green-500 focus:ring-2`" :name="$t('Last name')+':'" :error="form.errors.last_name" :type="'text'" :class="'col-span-6 md:col-span-3'"/>
                    </div>
                    <div class="mt-5">
                        <FormInput v-model="form.organization_name" inputIcon='<i class="pi pi-objects-column"></i>' :inputClass="`py-2.5 focus:border-green-200 focus:border-2 focus:ring-green-500 focus:ring-2`" :name="$t('Organization name')+':'" :error="form.errors.organization_name" :type="'text'" :class="'sm:col-span-6'"/>
                    </div>
                    <div class="mt-5">
                        <FormInput v-model="form.email" inputIcon='<i class="pi pi-at"></i>' :inputClass="`py-2.5 focus:border-green-200 focus:border-2 focus:ring-green-500 focus:ring-2`" :name="$t('Email')+':'" :error="form.errors.email" :type="'email'" :class="'sm:col-span-3'"/>
                    </div>
                    <div class="mt-5">
                        <FormPhoneInput v-model="form.phone" inputClass="py-1.5" :name="$t('Phone')+':'" :error="form.errors.phone" :type="'text'" :class="'sm:col-span-3'"/>
                    </div>
                    <div class="mt-5 grid gap-x-6 gap-y-4 grid-cols-6">
                        <FormInput v-model="form.password" inputIcon='<i class="pi pi-key"></i>' inputClass="py-1.5" :name="$t('Password')+':'" :error="form.errors.password" :type="'password'" :class="'col-span-6 md:col-span-3'" id="password" :eyeEnable="true" @viewPassword="viewPassword"/>
                        <FormInput v-model="form.password_confirmation" inputIcon='<i class="pi pi-lock"></i>' inputClass="py-1.5" :name="$t('Confirm password')+':'" :error="form.errors.password_confirmation" :type="'password'" :class="'col-span-6 md:col-span-3'" id="confirmPassword" :eyeEnable="true" @viewPassword="viewPassword"/>
                    </div>
                    <div v-if="form.errors.recaptcha_response" class="form-error text-[#b91c1c] text-xs">{{ form.errors.recaptcha_response }}</div>
                    <div class="mt-6">
                        <button v-if="!isLoading" type="submit" class="rounded-md bg-green-600 hover:bg-green-700 px-3 py-3 text-sm text-white shadow-sm w-full">{{ $t('Create account') }}</button>
