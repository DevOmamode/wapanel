...
class OrganizationService
{
    ...
    public function getMetaData($request){
        $organization = Organization::select('metadata')->where('id', $request['organization'])->firstOrFail();
        $orgMetaData = json_decode($organization->metadata);
        return $orgMetaData;
    }
    public function getProductAndWhatsappPaySettings($request){
        $organization = Organization::select('product_settings','whatsapp_pay_settings','api_url')->where('id', $request['organization'])->firstOrFail();
        $orgProductSettings = $organization['product_settings'] ? (object) $organization['product_settings'] : null;
        $orgWhatsappPaySettings = $organization['whatsapp_pay_settings'] ? (object) $organization['whatsapp_pay_settings'] : null;
        $orgPaymentDetailsApi = $organization['api_url'] ? $organization['api_url'] : null;
        return ['orgProductSettings' => $orgProductSettings,'orgWhatsappPaySettings' => $orgWhatsappPaySettings,'orgPaymentDetailsApi' => $orgPaymentDetailsApi];
    }
