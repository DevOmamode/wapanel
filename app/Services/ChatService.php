...
class ChatService
{
    ...
    public function getChatList($request, $uuid = null, $searchTerm = null)
    {
        ...
        if ($uuid !== null) {
            ...
            $contact = Contact::with(['lastChat', 'lastInboundChat', 'notes', 'contactGroup','contactLabelMemebers'])->where('uuid', $uuid)->first();
            ...
            $chats = [];
            $organization = Organization::select('product_settings')->where('id', $this->organizationId)->first();
            $orgProductSettings = $organization ? (object) $organization['product_settings'] : null;
            foreach ($chatLogs as $chatLog) {
                $chatLogRelatedEntities = $chatLog->relatedEntities;
                if($chatLog->relatedEntities->metadata && json_decode($chatLog->relatedEntities->metadata)->type == 'interactive' && json_decode($chatLogRelatedEntities->metadata)->interactive->type == "nfm_reply"){
                    $orderId = Order::select('id')->where('shipping_chat_id', $chatLogRelatedEntities->id)->first();
                    if($orderId) {
                        $chatLogRelatedEntities->order_id = $orderId->id;
                    }
                }
                $chats[] = array([
                    'type' => $chatLog->entity_type,
                    'value' => $chatLogRelatedEntities
                ]);
            }
            ...
            if (request()->expectsJson()) {
                ...
                return Inertia::render('User/Chat/Index', [
                    ...
                    'orgProductSettings' => $orgProductSettings,
